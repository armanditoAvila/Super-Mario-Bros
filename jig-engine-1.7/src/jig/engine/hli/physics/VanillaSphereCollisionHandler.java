 ...
 * 
 */
public abstract class VanillaSphereCollisionHandler
       <A extends VanillaSphere, B extends VanillaSphere>
	 ...
	 */
	public VanillaSphereCollisionHandler(final BodyLayer<A> layer1,
	...
	public void findAndReconcileCollisions() {
		...
		for (i = 0; i < l1s; i++) {
			...
			sphereL1Position = sphereLayer1.getCenterPosition();
			...
			for (j = 0; j < l2s; j++) {
				...
				dist2 = (sphereLayer1.getRadius() + sphereLayer2.getRadius())
						* (sphereLayer1.getRadius() + sphereLayer2.getRadius());

				if (sphereL1Position
						.distance2(sphereLayer2.getCenterPosition()) < dist2) {
					// Collision!
